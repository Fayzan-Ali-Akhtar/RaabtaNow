...
const ProfilePage = () => {
  ...
  const handleDeleteJobPost = async (jobId:any) => {
    try {
      const token = getAuthToken();
      await axios.delete(`${VITE_BACKEND_URL}/api/deletejobpost/${jobId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setUserJobPosts(userJobPosts.filter((job:any) => job.id !== jobId));
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        setResumes(((prev:any) => [...prev, res.data.resume] as any)); // 🔥 Add new one
      } catch (error) {
  ...
  const handleDeleteResume = async (resumeId: number) => {
    try {
      ...
      setResumes((prev) => prev.filter((r:any) => r.id !== resumeId));
              ...
              <div className="mt-8 space-y-4">
                {resumes.length === 0 ? (
                  ...
                  resumes.map((resume:any) => (
                    ...
                    >
                      <a
                        href={`${VITE_BACKEND_URL}${resume.file_url}`}
                        target="_blank"
