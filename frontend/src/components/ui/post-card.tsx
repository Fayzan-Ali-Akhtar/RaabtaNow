...
export function PostCard({
  ...
  const handleLike = async () => {
    try {
      await axios.post(`${VITE_BACKEND_URL}/api/toggle/${id}`, {}, {
        headers: {
  ...
  const fetchComments = async (postId: string) => {
    ...
    try {
      const res = await axios.get(`${VITE_BACKEND_URL}/api/postcomment/${postId}`, {
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${VITE_BACKEND_URL}/api/createcomment`, {
        job_id: id,
        content: newComment,
        author_id: user?.id,
      }, {
                  ...
                  <div className="space-y-3 max-h-48 overflow-y-auto">
                    {commentList.map((c:any) => (
                      <div key={c.id} className="border-b pb-2">
                        <p className="text-sm font-semibold">{c.author?.name || author?.name}</p> {/* ✅ */}
